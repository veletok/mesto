...
function setEventListeners(formElement, validationParams) {
  const inputList = formElement.querySelectorAll(validationParams.inputSelector);
  const submitButton = formElement.querySelector(validationParams.submitButtonSelector);

  inputList.forEach((inputElement) => {
  inputElement.addEventListener('input', () => {
      checkInputValidity(formElement, inputElement, validationParams);
      toggleButtonState(submitButton, formElement.checkValidity(), validationParams);
  });
});
}
